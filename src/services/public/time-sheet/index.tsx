...
export const GetTimeSheetHrd = (page: number, perpage: number, type: string, userId: string, dateStar: string, dateEnd: string) => Axios.get(process.env.BASE_URL+`/timesheetHrd?page=${page}&limit=${perpage}`+`${type === "" ? '' : `&type=${type}`+`${userId === "" ? '' : `$userId=${userId}`}`}`);
