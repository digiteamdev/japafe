...
import { GetAllEmploye, GetRole, EditUser } from "../../../services";
...
export const FormEditUser = ({ dataSelected, content, showModal }: props) => {
    ...
    const [userRoleRemove, setUserRoleRemove] = useState<any>([]);
	...
	const settingData = () => {
		let listDataRole: any = []
		dataSelected.userRole.map( (res: any) => {
            listDataRole.push({
                id_tbl: res.id,
                id: res.roleId,
        ...
        setUserRole(listDataRole)
        setData({
			employeeId: dataSelected.employee.id,
			username: dataSelected.username,
			hashed_password: "",
			confirm_password: "",
		});
        setEmploye(dataSelected.employee.employee_name);
		setEmployeId(dataSelected.employee.id);
		setDepartement(dataSelected.employee.sub_depart.name);
	};
	...
	const getEmploye = async () => {
		try {
			...
			if (response.data) {
				let listEmploye: any = response.data.result;
				listEmploye.push(dataSelected.employee);
				setDataEmploye(listEmploye);
			}
		} catch (error) {
			setDataEmploye([]);
		}
	...
	const editUser = async (payload: any) => {
		...
		userRole.map((item: any) => {
			role.push({ 
                id: item.id_tbl === undefined ? null : item.id_tbl,
                roleId: item.id 
            });
		});
		let dataPayload = {
			username: payload.username,
			hashed_password: payload.hashed_password === "" ? null : payload.hashed_password,
			role: role,
		};
		try {
			const response = await EditUser(dataSelected.id, dataPayload);
			if (response.data) {
