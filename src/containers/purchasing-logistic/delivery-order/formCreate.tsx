...
import { GetAllMRPo, GetAllCoa, AddPrMr, GetBom, AddDo } from "../../../services";
...
export const FormCreateDo = ({ content, showModal }: props) => {
    ...
    const addDo = async (payload: any) => {
        setIsLoading(true);
        try {
            const response = await AddDo(payload)
            if (response.data) {
				toast.success("Add Delivery Order Success", {
					position: "top-center",
					autoClose: 5000,
            ...
            toast.error("Add Delivery Order Failed", {
				position: "top-center",
				autoClose: 5000,
				hideProgressBar: true,
				closeOnClick: true,
								...
								<InputSelectSearch
									...
									onChange={(e: any) => {
                                        if(e.label === 'Internal'){
                                            ...
                                            setFieldValue('ship_to', e.value.customerPo?.quotations?.Customer?.name)
                                            setFieldValue('contact', e.value.customerPo?.quotations?.CustomerContact?.contact_person)
                                            setFieldValue('phone', e.value.customerPo?.quotations?.CustomerContact?.phone)
                                            setFieldValue('address', e.value.customerPo?.quotations?.Customer?.address[0]?.address_workshop)
                                        }
									}}
									required={true}
									withLabel={true}
												...
												>
													<div className='w-full'>
														<InputArea
															...
															type='text'
                                                            onChange={(e:any) => {
                                                                setFieldValue(`DOdetail.${i}.desc`, e.target.value)
                                                            }}
															value={res.desc}
															row={2}
															required={true}
													...
													<div className='w-full'>
														<Input
															...
															type='text'
                                                            onChange={(e:any) => {
                                                                setFieldValue(`DOdetail.${i}.unit`, e.target.value)
                                                            }}
															value={res.unit}
															required={true}
															withLabel={true}
													...
													<div className='w-full'>
														<InputArea
															...
															type='text'
                                                            onChange={(e:any) => {
                                                                setFieldValue(`DOdetail.${i}.note`, e.target.value)
                                                            }}
															value={res.note}
															row={2}
															required={true}
