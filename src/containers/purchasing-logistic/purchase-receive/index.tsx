...
import { FormCreatePurchaseReceive } from "./formCreate";
import { ViewPurchaseReceive } from "./view";
...
export const PurchaseReceive = () => {
	...
	useEffect(() => {
		getPurchaseReceive(page, perPage);
	...
	const getPurchaseReceive = async (page: number, perpage: number) => {
		setIsLoading(true);
		try {
			const response = await GetAllReceive(page, perpage);
			if (response.data) {
	...
	const searchPurchaseReceive = async (
		page: number,
		limit: number,
		search: string
	) => {
		setIsLoading(true);
		try {
			const response = await SearchReceive(page, limit, search);
			if (response.data) {
				setData(response.data.result);
			}
		} catch (error) {
