...
export const ViewSoSR = ({ dataSelected, content, showModal }: props) => {
	...
	const Ppn = (detail: any, tax: string) => {
		let totalBayar: any = Total(detail);
		let totalPPN: any = (totalBayar * ppn) / 100;
		return tax === 'ppn' || tax === 'ppn_and_pph' ? totalPPN.toString() : "0";
	};

	const Pph = (detail: any, tax: string) => {
		let totalBayar: any = Total(detail);
		let totalPPH: any = (totalBayar * pph) / 100;
		return tax === 'pph' || tax === 'ppn_and_pph' ? totalPPH.toString() : "0";
	};

	const GrandTotal = (detail: any, tax: string) => {
		let totalBayar: any = Total(detail);
		if (tax === 'ppn') {
			...
			let totalPPH: any = Pph(detail, tax);
	...
	const totalTermOfPayment = (data: any, detail: any, tax: string) => {
		let total: number = 0;
							...
							<Form>
									...
									<div className='w-full'>
										<InputSelectSearch
											...
											onChange={(e: any) => {
												...
												setPpn(e.value.supplier.ppn);
												setPph(e.value.supplier.pph);
												setData({
													dateOfPO: new Date(),
													idPO: values.idPO,
													ref: values.ref,
								...
								<FieldArray
									...
									render={(arrayPayment) => {
										return values.termOfPayment.map((res: any, i: number) => {
											return (
												<Section
													className='grid md:grid-cols-5 sm:grid-cols-3 xs:grid-cols-1 gap-2 border-b-[3px] border-b-green-500 pb-2 mt-2'
													key={i}
												>
													<div className='w-full'>
														<InputSelect
													...
													<div className='w-full'>
														{i === values.termOfPayment.length - 1 ? (
															<a
																className='inline-flex text-green-500 mr-6 mt-8 text-xl cursor-pointer'
																onClick={() => {
																	arrayPayment.push({
																		limitpay: "Normal",
																		percent: 0,
										...
										>
											<div className='w-full'>
												<Input
													...
													label='Job No / No SR'
													type='text'
													value={`${result.job_no} / ${result.no_sr}`}
													disabled={true}
													required={true}
													withLabel={true}
													className='bg-white border border-primary-300 text-gray-900 sm:text-sm rounded-lg block w-full p-2.5 outline-primary-600'
								...
								{values.detailMr.length > 0 ? (
									<div className='w-full'>
										...
										<Section className='grid md:grid-cols-6 sm:grid-cols-3 xs:grid-cols-1 gap-2'>
											<div className='w-full col-span-5'>
												<p className='text-xl mt-4 text-end'>PPH</p>
											</div>
											<div className='w-full'>
												<Input
													id='total'
													...
													placeholder='PPH'
													type='text'
													value={formatRupiah(Pph(values.detailMr, values.tax))}
													disabled={true}
													required={true}
													withLabel={true}
													className='bg-white border border-primary-300 text-gray-900 sm:text-sm rounded-lg block w-full p-2.5 outline-primary-600'
