...
import { ViewPurchaseDirectService } from "./view";
...
export const DirectPurchaseService = () => {
	...
	useEffect(() => {
		getPurchaseMR(page, perPage, "DSO");
	...
	const searchPurchaseMR = async (
		...
		setIsLoading(true);
		try {
			const response = await GetPurchaseDirrect(page, limit, "DSO");
			if (response.data) {
				setData(response.data.result);
			}
		} catch (error) {
				...
				{totalPage > 1 ? (
					<Pagination
						...
						onChangePage={(value: any) => {
							...
							getPurchaseMR(value, perPage, "DSO");
						}}
					/>
				) : null}
			</Content>
