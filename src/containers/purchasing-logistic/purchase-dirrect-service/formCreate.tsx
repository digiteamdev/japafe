...
export const FormCreatePurchaseDirrect = ({ content, showModal }: props) => {
	...
	const getPurchaseMR = async () => {
		setIsLoading(true);
		try {
			const response = await GetPurchaseDirrect(undefined, undefined, "DSO");
			if (response.data) {
				let detail: any = [];
				let suplier: any = [];
	...
	const getDetailPurchaseMR = async (id: string) => {
		setIsLoading(true);
		try {
			const response = await GetPurchaseSupplier(id, "DSO");
			if (response.data) {
								...
								<table className='w-full text-xs'>
									<thead>
										<tr>
											...
											<th className='text-center border border-black'>No SR</th>
											<th className='text-center border border-black'>
												Description
											</th>
