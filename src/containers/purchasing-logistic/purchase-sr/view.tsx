...
import { AddPrSr, BackToApprovalSr, GetAllSupplier } from "../../../services";
...
export const ViewPurchaseSR = ({ dataSelected, content, showModal }: props) => {
	...
	const backToApproval = async (data: any) => {
		try {
			...
			const response = await BackToApprovalSr(listData);
			if (response.data) {
				toast.success("Back to approval sr Success", {
					position: "top-center",
					autoClose: 5000,
					hideProgressBar: true,
					closeOnClick: true,
			...
			toast.error("Back to approval sr Failed", {
				position: "top-center",
				autoClose: 5000,
				hideProgressBar: true,
				closeOnClick: true,
															...
															<td className='pr-1 w-[20%]'>
																<InputSelectSearch
																	...
																	onChange={(e: any) => {
																		...
																		setFieldValue(`detailMr.${i}.supplier`, {
																			label: e.label,
																			value: e.value,
																		});
																	}}
