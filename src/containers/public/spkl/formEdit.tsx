...
import { GetEmployeDepartement, EditSpkl } from "@/src/services";
...
export const FormEditSpkl = ({ content, dataSelected, showModal }: props) => {
	const [employee, setEmployee] = useState<any>([]);
	const [isLoading, setIsloading] = useState<boolean>(false);
	const [deleteSpkl, setDeleteSpkl] = useState<any>([]);
	...
	const settingData = () => {
		let listSpkl: any = [];
		dataSelected.time_sheet_spkl?.map((res: any) => {
			listSpkl.push({
				id: res.id,
				spklId:res.spklId,
		...
		setData({
			...
			time_sheet_spkl: listSpkl,
		});
	};

	const editSpkl = async (payload: any) => {
		setIsloading(true);
		try {
			let data: any = {
				date: payload.date,
				time_sheet_spkl: payload.time_sheet_spkl,
				delete: deleteSpkl,
			};
			const response = await EditSpkl(data, dataSelected.id);
			if (response.data) {
				toast.success("Edit Spkl Success", {
					position: "top-center",
					autoClose: 5000,
    ...
    const removeSpkl = (id:string) => {
        let listRemove: any = deleteSpkl
        listRemove.push({
            id: id
        })
        setDeleteSpkl(listRemove)
    }
