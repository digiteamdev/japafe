...
import { FormCreateSpd } from "./formCreate";
import { ViewSpd } from "./view";
import { FormEditSpd } from "./formEdit";
import { DeleteSpd, GetSpd } from "../../../services";
...
export const SPD = () => {
	...
	const getSpd = async (page: number, perpage: number, search: string) => {
		setIsLoading(true);
		try {
			const response = await GetSpd(page, perpage, search);
			if (response.data) {
	...
	const deleteSpd = async (id: string) => {
		try {
			const response = await DeleteSpd(id);
			if (response.data) {
				toast.success("Delete SPD Success", {
					position: "top-center",
					autoClose: 5000,
		...
		<div className='mt-14 lg:mt-20 md:mt-20 sm:mt-20 xs:mt-24'>
						...
						data.map((res: any, i: number) => {
							return (
								...
								>
									...
									<td className='whitespace-pre-line p-1'>{res.tujuan}</td>
									<td className='whitespace-pre-line p-1 text-center'>
										{moment(res.berangkat).format("DD-MM-YYYY, h:mm a")}
									</td>
									<td className='whitespace-pre-line p-1 text-center'>
										{moment(res.kembali).format("DD-MM-YYYY, h:mm a")}
									</td>
			...
			{modalContent === "delete" ? (
				...
				>
					...
					modalContent === "add" ? (
						<FormCreateSpd content={modalContent} showModal={showModal} />
					) : (
						<FormEditSpd
							content={modalContent}
							showModal={showModal}
