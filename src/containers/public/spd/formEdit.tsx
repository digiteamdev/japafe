...
export const FormEditSpd = ({ dataSelected,content, showModal }: props) => {
    ...
    const settingData = () => {
        setData({
            ...
            selectCdv: {
                label: dataSelected?.cash_advance?.id_cash_advance,
                value: dataSelected?.cash_advance
            },
            departemen: dataSelected?.departement,
            tujuan: dataSelected?.tujuan,
            bulan: dataSelected?.bulan,
            date: new Date(dataSelected?.date),
            berangkat: new Date(dataSelected?.berangkat),
            kembali: new Date(dataSelected?.kembali),
            aktivitas: dataSelected?.aktivitas,
	...
	const editSpd = async (payload: any) => {
		setIsLoading(true);
		let body = {
			userId: idUser,
			cdvId: payload.cdvId,
		...
		try {
			const response = await EditSpd(body, dataSelected?.id);
			if (response.data) {
				toast.success("Edit Spd Success", {
					position: "top-center",
					autoClose: 5000,
						...
						<Section className='grid md:grid-cols-3 sm:grid-cols-1 xs:grid-cols-1 gap-2 mt-2'>
							...
							<div className='w-full'>
								<InputSelectSearch
									...
									label='Cash Advance'
                                    value={values.selectCdv}
									onChange={(e: any) => {
                                        setFieldValue("selectCdv", e);
