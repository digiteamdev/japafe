...
import { EditGolongan } from "../../../services";
...
export const FormEditGolongan = ({ content, dataSelected, showModal }: props) => {
	...
	const settingData = () => {
		setData({
            golongan: dataSelected?.golongan,
            huruf: dataSelected?.huruf,
            jabatan: dataSelected?.jabatan,
            t_keluarga: dataSelected?.t_keluarga,
            t_susu: dataSelected?.t_susu,
            t_jabatan: dataSelected?.t_jabatan,
            t_komunikasi: dataSelected?.t_komunikasi,
            t_kehadiran: dataSelected?.t_kehadiran,
            p_kedisiplinan: dataSelected?.p_kedisiplinan,
            uang_makan: dataSelected?.uang_makan,
            uang_transport: dataSelected?.uang_transport,
		});
	};

	const editGolongan = async (payload: data) => {
		setIsLoading(true);
		try {
            const body = {
			...
			const response = await EditGolongan(body, dataSelected?.id);
			if (response.data) {
				toast.success("Edit Golongan Success", {
					position: "top-center",
					autoClose: 5000,
			...
			toast.error("Edit Golongan Failed", {
				position: "top-center",
				autoClose: 5000,
				hideProgressBar: true,
				closeOnClick: true,
