...
import { FormCreateGolongan } from "./formCreate";
import { FormEditGolongan } from "./formEdit";
import { ViewGolongan } from "./view";
import { GetGolongan, DeleteGolongan } from "../../../services";
...
export const Golongan = () => {
	...
	useEffect(() => {
		getGolongan(page, perPage, search);
	...
	const getGolongan = async (page: number, perpage: number, search: string) => {
		setIsLoading(true);
		try {
			const response = await GetGolongan(page, perpage, search);
			if (response.data) {
	...
	const searchGolongan = async (
		page: number,
		limit: number,
		search: string
	) => {
        getGolongan(page, limit, search)
	};

	const deleteGolongan = async (id: string) => {
        try {
			const response = await DeleteGolongan(id);
			if(response.data){
				toast.success("Delete Golongan Success", {
					position: "top-center",
					autoClose: 5000,
			...
			toast.error("Delete Golongan Failed", {
				position: "top-center",
				autoClose: 5000,
				hideProgressBar: true,
				closeOnClick: true,
	...
	return (
		<div className='mt-14 lg:mt-20 md:mt-20 sm:mt-20 xs:mt-24'>
				...
				{totalPage > 1 ? (
					<Pagination
						...
						onChangePage={(value: any) => {
							...
							getGolongan(value, perPage, search);
						}}
					/>
				) : null}
			</Content>
			{modalContent === "delete" ? (
				...
				>
					{modalContent === "view" ? (
						<ViewGolongan dataSelected={dataSelected}  showModal={showModal}/>
					) : 
					modalContent === "add" ? (
                        <FormCreateGolongan content={modalContent} showModal={showModal} />
					) : (
						<FormEditGolongan content={modalContent} showModal={showModal} dataSelected={dataSelected}/>
					)}
				</Modal>
			)}
		</div>
