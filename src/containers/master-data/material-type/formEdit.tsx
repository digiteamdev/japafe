...
import { EditMaterialType } from '../../../services';
...
export const FormEditMaterialType = ({ content, showModal, dataSelected }: props) => {
    ...
    const editMaterialType = async (data: any) => {
		setIsLoading(true);
		try {
			const response = await EditMaterialType(data,dataSelected.id);
			if (response) {
				toast.success("Edit Material Type Success", {
					position: "top-center",
					autoClose: 5000,
			...
			toast.error("Edit Material Type Failed", {
				position: "top-center",
				autoClose: 5000,
				hideProgressBar: true,
				closeOnClick: true,
