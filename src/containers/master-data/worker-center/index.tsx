...
import { Grid, Eye, Edit, Trash2 } from "react-feather";
import { FormCreateWorkerCenter } from "./formCreate";
import {
	GetWorkerCenter,
	SearchWorkerCenter,
	DeleteWorkerCenter,
...
export const WorkerCenter = () => {
	...
	const headerTabel = [
		{ name: "No" },
		{ name: "Worker Center Name" },
		{ name: "Action" },
	];

	useEffect(() => {
		getWorkerCenter(page, perPage);
	...
	const getWorkerCenter = async (page: number, limit: number) => {
		setIsLoading(true);
		try {
			const response = await GetWorkerCenter(page, limit);
			if (response.data) {
	...
	const searchWorkerCenter = async (
		page: number,
		limit: number,
		search: string
	) => {
		setIsLoading(true);
		try {
			const response = await SearchWorkerCenter(page, limit, search);
			if (response.data) {
				setData(response.data.result);
				setIsLoading(false);
			}
	...
	const deleteWorkerCenter = async (id: string) => {
		try {
			const response = await DeleteWorkerCenter(id);
	...
	return (
		<div className='mt-14 lg:mt-20 md:mt-20 sm:mt-20 xs:mt-24'>
			<SectionTitle
				title='Worker Center'
				total={countData}
				icon={<Grid className='w-[36px] h-[36px]' />}
			/>
			<Content
				title='Worker Center'
				print={true}
			...
			</Content>
			{
				modalContent === 'delete' ? (
					...
					>
						{ modalContent === 'view' ? (
							<ViewWorkerCenter dataSelected={dataSelected} />
						) :  modalContent === 'edit' ? (
							<FormEditWorkerCenter content={modalContent} showModal={showModal} dataSelected={dataSelected}/>
						) : (
							<FormCreateWorkerCenter content={modalContent} showModal={showModal} dataSelected={dataSelected}/>
						)}
					</Modal>
				)
			}
