...
import { Calendar, Eye, Edit, Trash2 } from "react-feather";
import { FormCreateHoliday } from "./formCreate";
import {
	GetHoliday,
	SearchHoliday,
	DeleteHoliday,
...
export const Holiday = () => {
	...
	const getHoliday = async (page: number, limit: number) => {
		setIsLoading(true);
		try {
			const response = await GetHoliday(page, limit);
			if (response.data) {
	...
	const searchHoliday = async (
		...
		setIsLoading(true);
		try {
			const response = await SearchHoliday(page, limit, search);
			if (response.data) {
				setData(response.data.result);
				setIsLoading(false);
			}
	...
	const deleteHoliday = async (id: string) => {
		try {
			...
			if (response.status === 201) {
				toast.success("Delete Holiday Success", {
					position: "top-center",
					autoClose: 5000,
				...
				toast.error("Delete Holiday Failed", {
					position: "top-center",
					autoClose: 5000,
					hideProgressBar: true,
					closeOnClick: true,
	...
	return (
		<div className='mt-14 lg:mt-20 md:mt-20 sm:mt-20 xs:mt-24'>
			<SectionTitle
				title='Holiday'
				total={countData}
				icon={<Calendar className='w-[36px] h-[36px]' />}
			/>
			<Content
				title='Holiday'
				print={true}
							...
							return (
								...
								>
									...
									<td className='whitespace-nowrap px-6 py-4 w-[40%]'>
										{ moment(res.date_holiday).format('DD-MMMM-YYYY')}
									</td>
                                    <td className='whitespace-nowrap px-6 py-4 w-[40%]'>
										{res.description}
									</td>
			...
			</Content>
			{
				modalContent === 'delete' ? (
					...
					>
						{ modalContent === 'view' ? (
							<ViewHoliday dataSelected={dataSelected} />
						) :  modalContent === 'edit' ? (
                            <FormEditHoliday content={modalContent} showModal={showModal} dataSelected={dataSelected}/>
						) : (
							<FormCreateHoliday content={modalContent} showModal={showModal} dataSelected={dataSelected}/>
						)}
					</Modal>
				)
			}
