...
import { AddHoliday } from "../../../services/master-data";
...
export const FormCreateHoliday = ({
	dataSelected,
	content,
	showModal,
}: props) => {
	...
	const addHoliday = async (payload: any) => {
		setIsLoading(true);
		try {
			const response = await AddHoliday(payload.data);
			if (response) {
				toast.success("Add Holiday Success", {
					position: "top-center",
					autoClose: 5000,
			...
			toast.error("Add Holiday Failed", {
				position: "top-center",
				autoClose: 5000,
				hideProgressBar: true,
				closeOnClick: true,
										...
										{values.data.map((res, i) => (
											<div className="w-full">
												...
												{i === values.data.length - 1 ? (
													...
													>
														<Plus size={18} className='mr-1 mt-1' /> Add Holiday
													</a>
												) : null}
