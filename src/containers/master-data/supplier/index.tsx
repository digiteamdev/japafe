...
export const Supplier = () => {
	...
	const getSupplier = async (page: number, perpage: number) => {
		setIsLoading(true);
		try {
			const response = await GetSupplier(page, perpage);
			if (response.data) {
	...
	const searchSupplier = async (
		...
		setIsLoading(true);
		try {
			const response = await SearchSupplier(page, limit, search);
			if (response.data) {
				setData(response.data.result);
			}
		} catch (error) {
	...
	return (
		<div className='mt-14 lg:mt-20 md:mt-20 sm:mt-20 xs:mt-24'>
			...
			>
				{modalContent === "view" ? (
					<ViewSuplier dataSelected={dataSelected} />
					) : modalContent === "add" ? (
					<FormCreateSupplier content={modalContent} showModal={showModal} />
				) : (
					<FormEditSuplier content={modalContent} showModal={showModal} dataSuplier={dataSelected}/>
				)}
				</Modal>
			)}
		</div>
