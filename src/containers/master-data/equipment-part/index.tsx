...
import { SectionTitle, Content, Table, Modal, Button, ModalDelete } from "../../../components";
...
export const Equipment = () => {
	...
	const getEquipment = async (page: number, perpage: number) => {
		setIsLoading(true);
		try {
			const response = await GetEquipment(page, perpage);
			if (response.data) {
	...
	const searchEquipment = async (
		...
		setIsLoading(true);
		try {
			const response = await SearchEquipment(page, limit, search);
			if (response.data) {
				setData(response.data.result);
			}
		} catch (error) {
	...
	return (
		<div className='mt-14 lg:mt-20 md:mt-20 sm:mt-20 xs:mt-24'>
			<SectionTitle
				...
				total={countData}
				icon={<Package className='w-[36px] h-[36px]' />}
			/>
			<Content
				title='Equipment & Part'
				print={true}
			...
			>
				{modalContent === "view" ? (
					<ViewEquipment dataSelected={dataSelected} />
					) : modalContent === "add" ? (
						<FormCreateEquipment content={modalContent} showModal={showModal} />
				) : (
					<FormEditEquipment content={modalContent} showModal={showModal} dataEquipment={dataSelected}/>
				)}
				</Modal>
			)}
		</div>
