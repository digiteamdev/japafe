...
import { EditDepartement, DeleteSubDepartement } from "../../../services/master-data";
...
export const FormEditDepartement = ({
	dataSelected,
	content,
	showModal,
}: props) => {
    ...
    const deleteSubDepart = async (id: string) => {
		try {
			const response = await DeleteSubDepartement(id);
			if (response.data) {
				toast.success("Delete Sub Departement Success", {
					position: "top-center",
					autoClose: 5000,
					hideProgressBar: true,
					closeOnClick: true,
			...
			toast.error("Delete Sub Departement Failed", {
				position: "top-center",
				autoClose: 5000,
				hideProgressBar: true,
				closeOnClick: true,
									...
									<div key={i}>
										...
										{values.sub_depart.length !== 1 ? (
											<a
												...
												onClick={() => {
                                                    res.id !== "" ? deleteSubDepart(res.id) : null
													arraySub.remove(i);
												}}
											>
