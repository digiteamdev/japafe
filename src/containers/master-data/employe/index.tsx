...
import { ViewEmploye } from "./view";
import { FormEditEmploye } from "./formEdit";
import {
	GetAllDepartement,
	GetEmploye,
	SearchEmploye,
...
export const Employe = () => {
	...
	const getDepartement = async () => {
		try {
			...
			if (response.data) {
				setListDepartement(response.data.result);
			}
		} catch (error) {
			setListDepartement([]);
		}
	...
	const getEmploye = async (page: number, perpage: number) => {
		setIsLoading(true);
		try {
			const response = await GetEmploye(page, perpage);
			if (response.data) {
	...
	const searchEmploye = async (page: number, limit: number, search: string) => {
		setIsLoading(true);
		try {
			const response = await SearchEmploye(page, limit, search);
			if (response.data) {
				setData(response.data.result);
			}
		} catch (error) {
