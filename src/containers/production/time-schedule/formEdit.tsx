...
export const FormEditSchedule = ({
	...
	const [idWor, setIdWor] = useState<string>("");
	const [timeSchedulle, setTimeSchedulle] = useState<any>(new Date());
	const [holiday, setHoliday] = useState<string>("no");
	const [starDate, setStartDate] = useState<any>(new Date());
	const [endDate, setEndDate] = useState<any>(new Date());
	const [isLoading, setIsLoading] = useState<boolean>(false);
	const [isEdit, setIsEdit] = useState<boolean>(false);
	const [isShowGantt, setIsShowGantt] = useState<boolean>(true);
	...
	const settingData = () => {
		...
		let durationDay = countDay(
			dataSchedulle.wor.date_of_order,
			dataSchedulle.wor.delivery_date
		);
		setCustomer(dataSchedulle.wor.customerPo.quotations.Customer.name);
		setSubject(dataSchedulle.wor.subject);
		setJobDesc(dataSchedulle.wor.job_desk);
		setNote(dataSchedulle.wor.noted);
		setHoliday(dataSchedulle.holiday ? "yes" : "no");
		setStartDate(new Date(dataSchedulle.wor.date_of_order));
		setEndDate(new Date(dataSchedulle.wor.delivery_date));
		setData({
			idTs: dataSchedulle.idTs,
			worId: dataSchedulle.worId,
			timesch: dataSchedulle.timesch,
			holiday: false,
		});
		setDateHoliday(holidayDate);
		if (dataSchedulle.holiday) {
			for (var i = 0; i < durationDay; i++) {
				if (i === 0) {
					let unixTime = Math.floor(
						new Date(dataSchedulle.wor.date_of_order).getTime() / 1000
					);
		...
		newTask.push({
			start: new Date(dataSchedulle.wor.date_of_order),
			end: new Date(dataSchedulle.wor.delivery_date),
			name: dataSchedulle.wor.job_no,
			id: "Task",
			progress: 0,
		...
		dataSchedulle.aktivitas.map((res: any) => {
			let listDatesRange: any = [];
			let countHolidayRange = 0;
			let rangeDay = countDay(dataSchedulle.wor.date_of_order, res.startday);
			...
			newTask.push({
				...
				name: res.masterAktivitas.name,
				id: res.aktivitasId,
				idAktivitas: res.id,
				progress: res.progress,
				duration: res.days,
				holiday: res.holiday_count,
				color: "#60a5fa",
				left: 60 * rangeDay - 60,
				leftHoliday: 60 * (rangeDay - countHolidayRange) - 60,
				width: 60 * res.days,
				widthHoliday: 60 * res.days,
			});
		});
		setTask(newTask);
		showMonth(
			monthDiff(
				new Date(dataSchedulle.wor.date_of_order),
				new Date(dataSchedulle.wor.delivery_date)
			) + 1
		);
		showDate(dataSchedulle.wor.date_of_order, dataSchedulle.wor.delivery_date);
	};
	...
	const getWor = async () => {
		try {
			...
			if (response.data) {
				let listWors: any = [...response.data.result, dataSchedulle.wor];
				setListWor(listWors);
			}
		} catch (error: any) {
			setListWor([]);
	...
	const handleOnChanges = (event: any) => {
		if (event.target.name === "worId") {
			if (event.target.value !== "no data") {
				...
				if (data.id === dataSchedulle.worId) {
					settingData();
				} else {
					setTask([
						{
	...
	const showHoliday = (listDate: any) => {
		...
		listDate.map((res: any, i: number) => {
			holidayDate.map((result: any, idx: number) => {
				if (
	...
	const checkHoliday = (dataDate: any, use: string) => {
		let display = "";
		let count = 0;
		for (var i = 0; i < holidayDate.length; i++) {
			if (
				moment(holidayDate[i].date_holiday).format("DD-MMMM-YYYY") ===
										...
										listWor.map((res: any, i: number) => {
											return (
												<option
													...
													key={i}
													selected={
														res.id === dataSchedulle.worId ? true : false
													}
												>
													{res.job_no} -{" "}
											...
											{tasks.map((res: any, i: number) => {
												return (
													<div className='flex w-full' key={i}>
														<div className='w-[10%]'>
															<div
																className={`w-full border-l border-gray-500 text-justify m-auto h-14 p-2 ${
														...
														<div className='w-full'>
															<div className='grid grid-cols-4 w-full'>
																<div
																	className={`w-full border-l border-r border-gray-500 text-justify m-auto h-14 p-2 ${
																		i === tasks.length - 1 ? "border-b" : ""
																	} border-t`}
																>
																	<p className='text-center text-xs'>
																...
																<div
																	className={`w-full border-r border-gray-500 text-justify m-auto h-14 p-2 ${
																		i === tasks.length - 1 ? "border-b" : ""
																	} border-t`}
																>
																	<p className='text-center text-xs'>
																...
																? listDateHoliday.map((res: any, i: number) => {
																		return (
																			<div
																				...
																				className={`text-center border-r border-gray-500 p-4`}
																			>
																				<p className='p-[1px]'>&nbsp;</p>
																			</div>
																		);
