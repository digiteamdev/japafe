...
import { GetMrId, ApprovalDirectorMr } from "../../../services";
...
export const ViewApprovalMr = () => {
	...
	const approvalMR = async (status: string) => {
		...
		try {
			const response = await ApprovalDirectorMr(params.id, data);
			if (response.data) {
				toast.success(
					`${
				...
				router.push("/director/approvalMr");
			}
		} catch (error) {
			toast.error(
				`${
					...
					<div className='grid sm:grid-cols-1 md:grid-cols-2 gap-2'>
						<div className='flex items-center w-full'>
							...
							<div className='ml-[13px]'>
								<h1 className='text-3xl font-bold'>Approval MR</h1>
							</div>
						</div>
					</div>
